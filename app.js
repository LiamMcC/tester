...
app.post('/login', function(req,res){
 ...
 let query = db.query(sql2, (err, res2) =>{
  ...
  req.session.email = "LoggedInCon"
  }
  
  else{
   res.redirect("/reviews");
   req.session.destroy(session.email); // destroy the session if they log in correctly
   console.log("Session Killed as you cant log in -- Wrong Password")
  }
...
app.get('/createit', function(req,res){
 let sql = 'CREATE TABLE content (Id int NOT NULL AUTO_INCREMENT PRIMARY KEY, Name varchar(255), Image varchar(255), Activity varchar(255), Content varchar(255));'
 let query = db.query(sql,(err,res)=>{
  if (err) throw err;
  console.log(res);
